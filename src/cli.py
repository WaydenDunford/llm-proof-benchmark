...
from .dual import add_manual_proofs, bundle, import_chatgpt, math_shepherd
...
@app.command("add-manual-proofs")
def add_manual_proofs_command(proofs: Path = typer.Option(..., exists=True), directory: Path = typer.Option(..., exists=True)):
    """Add Markdown proofs generated by online models to an unevaluated local run."""
    target = add_manual_proofs(ROOT, proofs, directory)
    console.print(f"Combined proofs: {target}")


@app.command("evaluate-proofs")
def evaluate_proofs(proofs: Path = typer.Option(..., exists=True), math_shepherd_backend: str | None = None):
    """Run Math-Shepherd and make one blind ChatGPT bundle for an existing proofs file."""
    evaluations = math_shepherd(ROOT, proofs, math_shepherd_backend)
    markdown, _ = bundle(ROOT, proofs)
    console.print(f"Evaluations: {evaluations}\nExcel: {ROOT / 'results/benchmark_results.xlsx'}\nUpload to ChatGPT: {markdown}")
